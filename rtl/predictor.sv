...
module Predictor (
    ...
    input var Macroblock::Header header,
  ...
  function automatic byte unsigned dc_pred8(input logic tv, lv, input byte unsigned T[0:7],
    ...
    sum = 0;
    if (tv) begin
      for (int i = 0; i < 8; i++) sum += 32'(T[i]);
    end
    if (lv) begin
      for (int i = 0; i < 8; i++) sum += 32'(L[i]);
    end
    case ({
      tv, lv
    })
  ...
  always_comb begin
    ...
    case (header.intra_y_mode)
      IntraMBMode_DcPred: begin
        _dc16 = dc_pred16(top_valid, left_valid, top_y, left_y);
        for (int r = 0; r < 16; r++) for (int c = 0; c < 16; c++) pred_y[r][c] = _dc16;
      end

      IntraMBMode_VPred: begin
        if (top_valid) _row_val = top_y;
        else _row_val = '{default: 8'd127};
        for (int r = 0; r < 16; r++) for (int c = 0; c < 16; c++) pred_y[r][c] = _row_val[c];
      end

      IntraMBMode_HPred: begin
        if (left_valid) _col_val = left_y;
        else _col_val = '{default: 8'd129};
        for (int r = 0; r < 16; r++) for (int c = 0; c < 16; c++) pred_y[r][c] = _col_val[r];
      end

      IntraMBMode_TmPred: begin
        _T16 = top_valid ? top_y : '{default: 8'd127};
        _L16 = left_valid ? left_y : '{default: 8'd129};
        _P16 = (top_valid && left_valid) ? topleft_y : top_valid ? 8'd129 : 8'd127;
        for (int r = 0; r < 16; r++)
        for (int c = 0; c < 16; c++)
        pred_y[r][c] = clamp8(int'(_L16[r]) + int'(_T16[c]) - int'(_P16));
      end
  ...
  // BPred: 4*4 sub-block predictions
    ...
    if (header.intra_y_mode == IntraMBMode_BPred) begin
      ...
      for (int brow = 0; brow < 4; brow++) begin
        for (int bcol = 0; bcol < 4; bcol++) begin
          ...
          tl_avail = 1'b0;
          tr_avail = 1'b0;
          pred4    = '{default: '{default: 8'd128}};
          mode     = Macroblock::IntraBMode_BDcPred;
  ...
  function automatic void predict_4x4(output byte unsigned out[0:3][0:3],
    ...
    case (mode)
      ...
      IntraBMode_BTmPred: begin
        for (int r = 0; r < 4; r++)
        for (int c = 0; c < 4; c++) out[r][c] = clamp8(int'(L[r]) + int'(T[c]) - int'(TL));
      end
      ...
      IntraBMode_BLdPred: begin
        ...
        out[0][1] = avg3(a_ext[1], a_ext[2], a_ext[3]);
        out[1][0] = out[0][1];
        out[0][2] = avg3(a_ext[2], a_ext[3], a_ext[4]);
        out[1][1] = out[0][2];
        out[2][0] = out[0][2];
        out[0][3] = avg3(a_ext[3], a_ext[4], a_ext[5]);
        out[1][2] = out[0][3];
        out[2][1] = out[0][3];
        out[3][0] = out[0][3];
        out[1][3] = avg3(a_ext[4], a_ext[5], a_ext[6]);
        out[2][2] = out[1][3];
        out[3][1] = out[1][3];
        out[2][3] = avg3(a_ext[5], a_ext[6], a_ext[7]);
        out[3][2] = out[2][3];
      ...
      IntraBMode_BRdPred: begin
        ...
        e[7] = T[2];
        e[8] = T[3];
        out[3][0] = avg3(e[0], e[1], e[2]);
        out[3][1] = avg3(e[1], e[2], e[3]);
        out[2][0] = out[3][1];
        out[3][2] = avg3(e[2], e[3], e[4]);
        out[2][1] = out[3][2];
        out[1][0] = out[3][2];
        out[3][3] = avg3(e[3], e[4], e[5]);
        out[2][2] = out[3][3];
        out[1][1] = out[3][3];
        out[0][0] = out[3][3];
        out[2][3] = avg3(e[4], e[5], e[6]);
        out[1][2] = out[2][3];
        out[0][1] = out[2][3];
        out[1][3] = avg3(e[5], e[6], e[7]);
        out[0][2] = out[1][3];
      ...
      IntraBMode_BVrPred: begin
        ...
        e[7] = T[2];
        e[8] = T[3];
        out[3][0] = avg3(e[1], e[2], e[3]);
        out[2][0] = avg3(e[2], e[3], e[4]);
        out[3][1] = avg3(e[3], e[4], e[5]);
        out[1][0] = out[3][1];
        out[2][1] = avg2(e[4], e[5]);
        out[0][0] = out[2][1];
        out[3][2] = avg3(e[4], e[5], e[6]);
        out[1][1] = out[3][2];
        out[2][2] = avg2(e[5], e[6]);
        out[0][1] = out[2][2];
        out[3][3] = avg3(e[5], e[6], e[7]);
        out[1][2] = out[3][3];
        out[2][3] = avg2(e[6], e[7]);
        out[0][2] = out[2][3];
      ...
      IntraBMode_BVlPred: begin
        ...
        out[2][0] = avg2(a_ext[1], a_ext[2]);
        out[0][1] = out[2][0];
        out[1][1] = avg3(a_ext[1], a_ext[2], a_ext[3]);
        out[3][0] = out[1][1];
        out[2][1] = avg2(a_ext[2], a_ext[3]);
        out[0][2] = out[2][1];
        out[3][1] = avg3(a_ext[2], a_ext[3], a_ext[4]);
        out[1][2] = out[3][1];
        out[2][2] = avg2(a_ext[3], a_ext[4]);
        out[0][3] = out[2][2];
        out[3][2] = avg3(a_ext[3], a_ext[4], a_ext[5]);
        out[1][3] = out[3][2];
      ...
      IntraBMode_BHdPred: begin
        ...
        out[2][0] = avg2(e[1], e[2]);
        out[3][2] = out[2][0];
        out[2][1] = avg3(e[1], e[2], e[3]);
        out[3][3] = out[2][1];
        out[2][2] = avg2(e[2], e[3]);
        out[1][0] = out[2][2];
        out[2][3] = avg3(e[2], e[3], e[4]);
        out[1][1] = out[2][3];
        out[1][2] = avg2(e[3], e[4]);
        out[0][0] = out[1][2];
        out[1][3] = avg3(e[3], e[4], e[5]);
        out[0][1] = out[1][3];
      ...
      IntraBMode_BHuPred: begin
        ...
        out[0][2] = avg2(lv[1], lv[2]);
        out[1][0] = out[0][2];
        out[0][3] = avg3(lv[1], lv[2], lv[3]);
        out[1][1] = out[0][3];
        out[1][2] = avg2(lv[2], lv[3]);
        out[2][0] = out[1][2];
        out[1][3] = avg3(lv[2], lv[3], lv[3]);
        out[2][1] = out[1][3];
        out[2][2] = lv[3];
        out[2][3] = lv[3];
        out[3][0] = lv[3];
