pub fn predict_dcpred<const N: usize>(
    ...
    let dcval = match (top, left) {
        (Some(t), Some(l)) => {
            let sum_top: usize = t.iter().map(|&v| v as usize).sum();
            let sum_left: usize = l.iter().map(|&v| v as usize).sum();
            ((sum_top + sum_left + N) / (2 * N)) as u8 // rounding per spec
        }
        (Some(t), None) => {
            let sum_top: usize = t.iter().map(|&v| v as usize).sum();
            ((sum_top + (N / 2)) / N) as u8
...
pub fn predict_tmpred<const N: usize>(
    ...
    let t = top.as_ref().unwrap_or(&[127u8; N]);
    let l = left.as_ref().unwrap_or(&[129u8; N]);
    let p = *top_left.as_ref().unwrap_or(&127u8);

    let mut out = [[0u8; N]; N];
    for y in 0..N {
        for x in 0..N {
            let pred_i16 = (l[y] as i16) + (t[x] as i16) - (p as i16);
